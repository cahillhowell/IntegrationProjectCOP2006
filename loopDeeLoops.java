...
public class loopDeeLoops {
  public static void loops() {
    ...
    // precautionary statement so that the program doesn't break if the user types in something
    // unexpected
    else {
    ...
    /*
     * //This code works, but for some reason it continues to break the program, I think its the
     * nature of the do/while //that breaks the program, even a break; didn't fix the issue, but it
     * does run on its own!
     * 
    ...
    String breakLoop =
        "This code will count and stop at 3 and not reach 4 or keep counting, because of a break.";
    ...
    String continueLoop =
        "This code will not stop at 4 it will finish counting once it reachs 10, because its told to continue past 4 this time.";
    ...
    while (u <= 10) {
      if (u == 4) { // here is an example of a relational operator the 'Equal to'
        ...
        continue; // where as this is not necessary as if its not there, it will continue anyways,
                  // its more useful for a more intricate program idea, continue says if this is met
                  // keep going
      }
      System.out.print(u);
      u++;
